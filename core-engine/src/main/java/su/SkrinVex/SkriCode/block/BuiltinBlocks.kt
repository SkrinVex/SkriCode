...
object BlockFactory {
    fun create(type: String): BlockDef? = when (type) {
        ...
        "comment" -> build("comment", "Комментарий", "Заметка для себя — никак не влияет на выполнение скрипта", BlockCategory.COMMENT, listOf(
            "text" to p("", "Текст заметки", "Например: почему этот цикл тут нужен")))
        ...
        "call_func" -> build("call_func", "Вызвать функцию", "Вызывает пользовательскую функцию с аргументами", BlockCategory.FUNCTION, listOf(
            "name"       to p("", "Имя функции", "Имя вызываемой функции"),
            "args"       to p("", "Аргументы", "Значения через запятую, напр. 10, {x} + 2"),
            "return_var" to p("", "Сохранить результат в", "Переменная для сохранения возвращённого значения (опционально)")))

        "return_val" -> build("return_val", "Вернуть значение", "Возвращает результат вычисления и завершает функцию", BlockCategory.FUNCTION, listOf(
            "value" to p("0", "Значение", "Возвращаемое значение или выражение, напр. {a} + {b}")))
